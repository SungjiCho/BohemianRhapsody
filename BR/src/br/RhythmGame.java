...
public class RhythmGame extends Thread{
	...
	public void dropNotes(String titleName) {
		...
		if(titleName.equals("Queen - Love Of My Life")){
			int startTime = 8360 - (Main.REACH_TIME*1000);  //처음 비트가 떨어지는 게 약 8초
			int gap = 125;  //8분 음표
			beats = new Beat[] {
...
//					new Beat(startTime + gap * 24, "Space")
		...
		}else if(titleName.equals("Queen - Bohemian Rhapsody")) {
		...
		}else if(titleName.equals("Queen - We Are The Champion")) {
			int startTime = 4460 - (Main.REACH_TIME*1000);
			int gap = 125;  //8분 음표
			beats = new Beat[] {
					new Beat(startTime, "S"),
